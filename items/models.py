...
是整个系统的数据基础。所有模型均继承自 Django 的 models.Model 类，使用 Django ORM 进行数据库操作。
...
class Profile(models.Model):
    ...
    实现了新用户注册后的审批机制, 只有管理员批准的用户才能发布物品。
    ...
    # 给模型增加 __str__() 方法，能在命令行里使用带来方便，Django 自动生成的 admin 里也使用这个方法来表示对象
    def __str__(self):
        """返回用户资料的字符串表示"""
